...
describe('.forEach()', () => {
  test('object is empty', () => {
    ...
    dictionary(object).forEach((value, key) => { r1.push([key, value]) })
    expect(r1).toStrictEqual([])

    const r2: Array<[string, number]> = []
    Dictionary.from(object).forEach((value, key) => { r2.push([key, value]) })
    expect(r2).toStrictEqual([])

    const r3: Array<[string, number]> = []
    new Dictionary(object).forEach((value, key) => { r3.push([key, value]) })
