...
describe('.includesOneOf()', () => {
  test('object is empty', () => {
    ...
    for (const { values, expected } of data) {
      ...
      const r1 = dictionary(object).includesOneOf(...values)
      expect(r1).toBe(expected)

      const r2 = Dictionary.from(object).includesOneOf(...values)
      expect(r2).toBe(expected)

      const r3 = new Dictionary(object).includesOneOf(...values)
